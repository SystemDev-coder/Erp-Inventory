...
export const UserPrivilegesTab = ({
  ...
  // NEW: Derive overrides by comparing desired effective state with inherited state
      ...
      {!!userId && !!selectedUser && (
        <div className="rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200">
          ...
          <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            This saves only to <span className="font-mono">ims.user_permission_overrides</span>.
          </div>
        </div>
      )}
