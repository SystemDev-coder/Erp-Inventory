...
import { CheckSquare, Lock, Pencil, Plus, Save, Search, Shield, Trash2, Users, XSquare } from 'lucide-react';
...
const System = () => {
  ...
  const [privilegesRoleId, setPrivilegesRoleId] = useState<number | null>(null);
  const [rolePermissions, setRolePermissions] = useState<RolePermission[]>([]);
  const [loadingRolePermissions, setLoadingRolePermissions] = useState(false);
  const [savingRolePermissions, setSavingRolePermissions] = useState(false);
  const [privilegesSearch, setPrivilegesSearch] = useState('');
  ...
  // NEW: Gate System actions so only authorized users see buttons/actions
  ...
  const canViewPermissions = hasAnyPerm(['system.permissions.manage', 'permissions.view']);
  ...
  // NEW: Load current role permissions for Privileges tab
  const loadPrivileges = useCallback(
    async (roleId: number) => {
      ...
      const res = await systemService.getRolePermissions(roleId);
      if (res.success && res.data?.permissions) {
    ...
    [permissions.length, roles.length, showToast]
  );

  // NEW: Toggle a permission on/off in local state (saving happens via Save button)
  ...
  // NEW: Bulk toggle all currently visible permissions
  const setAllVisiblePermissions = (enabled: boolean, visible: RolePermission[]) => {
    const visibleSet = new Set(visible.map((p) => p.perm_id));
    setRolePermissions((prev) =>
      prev.map((p) => (visibleSet.has(p.perm_id) ? { ...p, has_permission: enabled } : p))
    );
  };

  // NEW: Persist role permissions (replaces full set on backend)
  const savePrivileges = async () => {
    if (!privilegesRoleId) {
      showToast('error', 'Privileges', 'Select a role first');
      return;
    }
    ...
    const res = await systemService.updateRolePermissions(privilegesRoleId, permIds);
    if (!res.success) {
    ...
    await loadPrivileges(privilegesRoleId);
    setSavingRolePermissions(false);
  };
  ...
  // NEW: Derived role-permissions view (search + grouping) for Privileges tab
  const filteredRolePermissions = useMemo(() => {
    const q = privilegesSearch.trim().toLowerCase();
  ...
  const tabs = [
    {
      ...
      content: (
	    ...
	    // NEW: Role privileges matrix (admin assigns permissions per role)
	      ...
	      ? [
	          {
	            ...
	            label: 'Privileges',
	            icon: CheckSquare,
	            badge: rolePermissions.filter((p) => p.has_permission).length,
	            content: (
	              <div className="space-y-3">
	                <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
	                  <div className="flex flex-wrap items-center gap-3">
	                    <label className="text-sm font-medium text-slate-700 dark:text-slate-200">
	                      ...
	                      <select
	                        ...
	                        value={privilegesRoleId ?? ''}
	                        onChange={async (e) => {
	                          const nextId = Number(e.target.value || 0) || null;
	                          setPrivilegesRoleId(nextId);
	                          if (nextId) await loadPrivileges(nextId);
	                        }}
	                      >
	                    ...
	                    <div className="relative">
	                      ...
	                      <input
	                        ...
	                        value={privilegesSearch}
	                        onChange={(e) => setPrivilegesSearch(e.target.value)}
	                  ...
	                  <div className="flex flex-wrap items-center gap-2 justify-end">
	                    <button
	                      type="button"
	                      className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-sm hover:bg-slate-50 dark:hover:bg-slate-800"
	                      onClick={() => setAllVisiblePermissions(true, filteredRolePermissions)}
	                      disabled={!privilegesRoleId || loadingRolePermissions}
	                    >
	                    ...
	                    <button
	                      type="button"
	                      className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-sm hover:bg-slate-50 dark:hover:bg-slate-800"
	                      onClick={() => setAllVisiblePermissions(false, filteredRolePermissions)}
	                      disabled={!privilegesRoleId || loadingRolePermissions}
	                    >
	                      <XSquare className="h-4 w-4" /> Uncheck visible
	                    </button>
	                    {/* UPDATED: Only users with update permission can save */}
	                    {canUpdateRolePermissions && (
	                      <button
	                        ...
	                        disabled={!privilegesRoleId || loadingRolePermissions || savingRolePermissions}
	                      >
	                        <Save className="h-4 w-4" /> {savingRolePermissions ? 'Saving...' : 'Save'}
	                      </button>
	                    )}
	                  </div>
	                </div>
	                ...
	                {!privilegesRoleId ? (
	                  ...
	                  <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
	                    {groupedRolePermissions.map(([groupTitle, groupPerms]) => (
	                          ...
	                          {groupPerms.map((p) => (
	                            ...
	                            >
	                              <input
	                                ...
	                                onChange={() => toggleRolePermission(p.perm_id)}
