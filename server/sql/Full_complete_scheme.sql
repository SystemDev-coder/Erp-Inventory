...
CREATE TABLE IF NOT EXISTS ims.inventory_transaction (
    ...
    transaction_type VARCHAR(12) NOT NULL CHECK (transaction_type IN ('IN','OUT','ADJUSTMENT')),
    ...
    notes          TEXT,
    status         VARCHAR(12) NOT NULL DEFAULT 'POSTED' CHECK (status IN ('POSTED','PENDING','CANCELLED')),
    created_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at     TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
name VARCHAR(120) NOT NULL,
created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
CONSTRAINT uq_exp_type_branch_name UNIQUE (branch_id, name)
);
