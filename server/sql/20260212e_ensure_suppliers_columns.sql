-- Ensure suppliers has all current columns (company_name etc.)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema='ims' AND table_name='suppliers' AND column_name='company_name'
  ) THEN
  ...
  -- Drop unused legacy column if still present
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
